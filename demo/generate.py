"""Generate a synthetic raw.json for a fictional account so the repo ships with a demo.
...
"""
import argparse, datetime as dt, json, os, random

D = os.path.dirname(os.path.abspath(__file__))
ACCOUNT = "octodev"
REPOS = {"acme/api": 0.34, "acme/web": 0.26, "acme/infra": 0.15, "acme/data-pipeline": 0.12,
         "octodev/dotfiles": 0.05, "opensource/toolkit": 0.08}
HOLIDAYS = {"12-24", "12-25", "12-26", "12-31", "01-01"}
VACATION = [("2026-02-16", "2026-02-20"), ("2026-07-06", "2026-07-24"), ("2025-10-13", "2025-10-17")]
...
def main():
    ...
    for s, e in VACATION:
        ...
        while d <= dt.date.fromisoformat(e):
            off.add(d); d += dt.timedelta(1)

    def iso(t):
        return t.strftime("%Y-%m-%dT%H:%M:%SZ")

    def work_time(day):
        # local 08:00-19:00 with a lunch dip; stored as UTC-1 (a "Europe" flavoured account)
        h = rnd.choice([8, 9, 9, 10, 10, 10, 11, 11, 13, 14, 14, 15, 15, 16, 16, 17, 18])
        return dt.datetime.combine(day, dt.time(h - 1, rnd.randrange(60), rnd.randrange(60)))
    ...
    while day <= end:
        ...
        for _ in range(n):
            ...
            closed = merged
            prs.append("\t".join([str(num[repo]), iso(created), iso(closed),
                                  iso(merged) if is_merged else "", repo]))
            if is_merged:
                size = int(rnd.lognormvariate(4.3, 1.4))
                if rnd.random() < 0.015:
                    size = rnd.randrange(12000, 60000)
                add = int(size * rnd.uniform(0.45, 0.9))
                loc[repo].append({"number": num[repo], "createdAt": iso(created), "mergedAt": iso(merged),
                                  "additions": add, "deletions": size - add,
                                  "changedFiles": max(1, int(size ** 0.55 * rnd.uniform(0.3, 1.2)))})
        for _ in range(3):
            if rnd.random() < base * 0.18:
                t = work_time(day)
                issues.append("\t".join(["0", iso(t), "", "", rnd.choice(list(REPOS))]))
        day += dt.timedelta(1)

    out = {"account": ACCOUNT, "start": start.isoformat(), "end": end.isoformat(),
           "prs": prs, "issues": issues, "loc": loc,
